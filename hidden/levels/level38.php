...
//maybe do this.moveState in case you decide to use shift for sprinting which would be ecaulated in the move function
	that.still = false;
								setDxDyToObject(that, to);
}else {
that.still = true; 
...
//objects are better because I might not be using constructors at all. That was the main idea. I will only use these functions that make presets when I seem to be doing something over and over again. also in a weird way it is less confusing: how is this different than just using the 'this' section of a constructor for functions? cannot copy and paste object. not really a problem.
//I want to call functions of a type, like of the fire type.
//just have one function that reads parameters
//much like my current idea for guns
//section(like fire)-> selector (like evalGuns) -> event(like always or on mouse down which read globals or comparisons) -> 
//warp could 
//or you could append the functions to a function
/*
function CircleShot(whose){
return { whose : whose
init : function{matchXYTo(this, whose); matchTrackTargetTo(this, whose)}
function matchXYTo(that, to){
	that.x = to.x;
	that.y = to.y;
}
function matchTrackTargetTo(that, to){
	that.x = to.x;
	that.y = to.y;
}
	*/
