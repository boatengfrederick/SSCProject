...
var mouse = new Mouse();
								...
								var cursors = [cursor];
                //player
///player depends on view for its draw functiond
...
//movement order matters for viewport versus player
                var instances = [players, views, cursors, rectangles, spitters, circleShots];
								//*******METHODS*********
//death methods
...
function trackNearObject(that, to, bounceRange){
			if( getDistanceObjs(that, to) >(bounceRange||100)){
								setDxDyToObject(that, to);
//console.log(view.x, cursor.x);
}}	
function trackStopNearObject(that, to, range){
			if( getDistanceObjs(that, to) >(range||100)){
								setDxDyToObject(that, to);
}else {
that.still = true; 
;}
}	
function matchStopNearObject(that, to, range){
console.log(that, to);
			if( getDistance(that.x +that.dx, that.y +that.dy, to.x, to.y) >(range||100)){
								matchMoveFunc(that, to);
}else {
that.still = true; 
;}
}	
...
function matchMoveFunc(that, to){
							if((to.still || false) != true){
that.dx = to.dx;
that.dy = to.dy;
that.y += to.dy
console.log(that, to)
...
//the or Does not work
                ...
                this.y = canvas.height/2;
								...
								//View.prototype.eventDirection = matchMove(this, player);
//when player move diagonal there is a chance that where the player will be would not be outside the range of the viewport
//DONT USE THIS>>>>
								View.prototype.track = function(){matchStopNearObject(this, this.trackTarget, this.followRange);};
...
//TODOdone I think
								Cursor.prototype.eventDirection = matchMove(cursor, view);
...
//weird but works moveing the mouse takes it off viewport and moves to mouse, the added cursor dx puts it back in line
