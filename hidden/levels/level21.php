...
var ball1 = new Ball(200, 150, 10, -90, 7);
var ball2 = new Ball(200, 100, 10, -90, 2);
...
//canvas x and y increase as you go towards the bottom right **fixed**
	...
	var radiansToCurrent = radiansToNext + Math.PI;
/*
	var dxToCollision = balls[i].radius * Math.cos(radiansToNext);
	var dyToCollision = balls[i].radius * Math.sin(radiansToNext);
	var xOfCollision = balls[i].x + dxToCollision;
	var yOfCollision = balls[i].y + dyToCollision;
*/
	var normalX = Math.cos(radiansToNext);
	var normalY = Math.sin(radiansToNext);
	var nextNormalX = Math.cos(radiansToCurrent);
	var nextNormalY = Math.sin(radiansToCurrent);
	var newDx = ((normalY*normalY) - (normalX*normalX))*balls[i].dx - (2*normalX*normalY)*balls[i].dy;
	var newDy = ((normalX*normalX) - (normalY*normalY))*balls[i].dy - (2*normalY*normalX)*balls[i].dx;
	var newDxNext = ((nextNormalY*nextNormalY) - (nextNormalX*nextNormalX))*balls[j].dx - (2*nextNormalX*nextNormalY)*balls[j].dy; 
	var newDyNext = ((nextNormalX*nextNormalX) - (nextNormalY*nextNormalY))*balls[j].dy - (2*nextNormalY*nextNormalX)*balls[j].dx; 
	var newRadians = Math.atan2(newDy, newDx);
	var newRadiansNext = Math.atan2(newDyNext, newDxNext);

	balls[i].angle = Math.round((newRadians * (180/Math.PI)) * 100) / 100;
	balls[j].angle = Math.round((newRadiansNext * (180/Math.PI)) * 100) / 100;
	message.innerHTML = balls[i].angle + " small " + balls[j].angle + " big";
	}else{
